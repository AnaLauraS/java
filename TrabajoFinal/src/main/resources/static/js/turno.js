...
// crear nuevo
    ...
    guardar.addEventListener('submit', function(e){
        ...
        fetch(url, settings)
            ...
            .then(data => {
                if (data) {
                    document.querySelector('.reservado').innerHtml=`Se reservó el turno con ID ${data.id}, con el odontologo ${data.odontologo.nombre} ${data.odontologo.apellido}, para el paciente ${data.paciente.nombre} ${data.paciente.nombre}. Fecha y hora: ${data.diaHora}`
                }
            })
            .catch (error => {
                alert('Error: ', error);
